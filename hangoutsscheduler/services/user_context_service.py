...
from hangoutsscheduler.models.message_context import MessageContext, MessageContextChatHistory
...
class UserContextService:
    ...
    USER_CHAT_HISTORY_DESCRIPTION = "Chat history for the specific user across all their channels."
    ...
    def resolve_chat_history(self, session: Session, username: str, new_message: ChatMessage) -> MessageContext:
        chat_history = self._get_or_create_chat_history(session, username)
        self._update_history(chat_history, new_message)
        session.commit()
        user_chat_history = MessageContextChatHistory(name=self.USER_CHAT_HISTORY_NAME, description=self.USER_CHAT_HISTORY_DESCRIPTION, messages=chat_history.history)
        return MessageContext(message=new_message.content, username=username, histories=[user_chat_history])

    def update_with_llm_response(self, session: Session, username: str, response: ChatMessage):
        chat_history = self._get_or_create_chat_history(session, username)
